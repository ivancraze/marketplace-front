...
interface IInputItselfProps {
    type: 'text' | 'password' | 'date';
    isGhost: boolean;
    disabled?: boolean;
    icon?: React.ReactNode;
}
export const InputItselfStyled = styled.input<IInputItselfProps>`
    width: 100%;
    color: #5e5873;
...
interface ITogglePasswordVisibilityProps {
    passwordVisibility: boolean;
}
export const TogglePasswordVisibilityStyled = styled.div<ITogglePasswordVisibilityProps>`
    position: absolute;
    bottom: 13px;
    right: 15px;
    width: 15px;
